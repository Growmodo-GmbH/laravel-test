        ...
        <div class="row">
            ...
            <div class="col-md-12" style="padding-top: 25px;">
                ...
                <div v-if="result.user.username == 'admin'">
                    <button @click="$router.push('/admin/users')" type="button" class="btn btn-success">View Records</button>
                    <button @click.prevent="handleLogout" type="button" class="btn btn-danger">Log Out</button>
                </div>

                <div v-else>
                    <button @click.prevent="handleUnsubscribe" type="button" class="btn btn-secondary">Unsubscribe</button>
                    <button @click.prevent="handleLogout" type="button" class="btn btn-danger">Log Out</button>
                </div>
            </div>
        </div>
...
export default {
  ...
  methods: {
    ...
    handleUnsubscribe () {
      axios.post('http://127.0.0.1:8000/api/delete/' + this.result.user.id)
        .then(
          response => {
            console.log(response)
            localStorage.removeItem('token')
