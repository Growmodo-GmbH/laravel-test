<template>
    <div class="container">
        <div class="col-md-12">
            <button @click="$router.push('/admin/users')" type="button" class="btn btn-success" style="float: right;">Back to Records</button>
        </div>
            ...
            <form @submit.prevent="saveData">
                <div class="user-details">
                    <div class="input-box">
                        ...
                        <input type="text" v-model="result.username" name="username" id="username" disabled>
                    </div>
                    <div class="input-box">
...
export default {
  ...
  methods: {
      ...
      axios.get('http://127.0.0.1:8000/api/admin/user/' + this.userID)
        .then(
          response => {
            ...
            $('#username').value(this.result.user.username)
          }
        )
        .catch(
          errors => {
    ...
    saveData () {
      axios.post('http://127.0.0.1:8000/api/update/' + this.result.id, this.result)
        .then(
          ({ data }) => {
            try {
              this.errorMessage = 'User info successfully updated!'
              return true
            } catch (err) {
              localStorage.removeItem('token')
              this.$router.push({ name: 'Login' })
